...
func (r *LocalVolumeReconciler) processNewSymlink(ctx context.Context, scName string, diskLocation *internal.DiskLocation, mountPointMap sets.Set[string]) (bool, error) {
	...
	if found {
		...
		pvSymlinkPath, err := currentDeviceInfo.RecoverPVSymlinkPath(ctx, symLinkDirPath, source, r.Client)
		if err != nil {
...
func (r *LocalVolumeReconciler) processRejectedDevicesForDeviceLinks(ctx context.Context, rejectedDevices []internal.BlockDevice, diskConfig *DiskConfig) map[string]int {
	...
	for storageClassName, disks := range diskConfig.Disks {
		...
		for _, devicePath := range devicePaths {
			...
			if err != nil {
				if lvdl, symlinkPath, ok := common.PolicyNotPreferredLVDL(err); ok {
					if _, updateErr := r.deviceLinkHandler.UpdateDeviceLinks(ctx, lvdl, blockDevice, lvdl.Status.CurrentLinkTarget, symlinkPath); updateErr != nil {
						klog.ErrorS(updateErr, "failed to update LVDL PreferredLinkTarget", "lvdl", lvdl.Name)
					}
				}
