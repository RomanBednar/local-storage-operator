...
func (r *LocalVolumeSetReconciler) processNewSymlink(
	...
	// there should usually never be a current device for this symlinkSource path
	...
	if found {
		symlinkPath, err = currentDeviceInfo.RecoverPVSymlinkPath(ctx, symLinkDir, symlinkSourcePath, r.Client)
		if err != nil {
			if lvdl, pvSymlinkPath, ok := common.PolicyNotPreferredLVDL(err); ok {
				if _, updateErr := r.deviceLinkHandler.UpdateDeviceLinks(ctx, lvdl, blockDevice, lvdl.Status.CurrentLinkTarget, pvSymlinkPath); updateErr != nil {
					klog.ErrorS(updateErr, "failed to update LVDL PreferredLinkTarget", "lvdl", lvdl.Name)
...
func (r *LocalVolumeSetReconciler) reportProvisioningFailure(lvSet *localv1alpha1.LocalVolumeSet, devName string, err error) {
	msg := fmt.Sprintf("provisioning failed for %s: %v", devName, err)
	r.eventReporter.Report(lvSet, newDiskEvent(diskmaker.ErrorProvisioningDisk, msg, devName, corev1.EventTypeWarning))
	klog.Error(msg)
}
...
func (r *LocalVolumeSetReconciler) createSymlinkAndSyncPVAndLVDL(
	...
	// this checks in root of /mnt/local-storage directory
